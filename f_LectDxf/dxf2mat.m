function C = dxf2mat(nomArch) 
if 0 
%% BASE 
C = dxf2mat('frst.dxf'); 
figure(1);axis equal; 
for i=1:C.llin 
line(C.lin{i}([1 4]),C.lin{i}([2 5]),'Color','green'); 
end 
for i=1:C.lcir 
rectangle('Position',[C.cir{i}([1 2])-C.cir{i}(3) ... 
2*C.cir{i}(3) 2*C.cir{i}(3)],... 
'Curvature',[1,1],'EdgeColor','red'); 
end 
for i=1:C.ltex 
text(C.tex{i,2}(1),C.tex{i,2}(2),C.tex{i,1},... 
'FontSize',C.tex{i,2}(3)*4,'Color','green',... 
'HorizontalAlignment','center'); 
end 
%% BASE 
C = dxf2mat('Device02b.dxf'); 
figure;axis equal; 
for i=1:C.llin 
line(C.lin{i}([1 4]),C.lin{i}([2 5]),'Color','green'); 
end 
...
for i=1:C.ltex 
text(C.tex{i,2}(1),C.tex{i,2}(2),C.tex{i,1},... 
'FontSize',C.tex{i,2}(3)/18/1.5,'Color','green'); 
end 
end
...
%case Add Entities 
case {'TEXT', 'MTEXT'} 
c_Tex{iTex,1}=f_ValGrCode(1,m_GrCodeEnt,c_ValAsocEnt); 
c_Tex{iTex,2}=[str2double(f_ValGrCode(10,m_GrCodeEnt,c_ValAsocEnt)),... 
str2double(f_ValGrCode(20,m_GrCodeEnt,c_ValAsocEnt)),... 
str2double(f_ValGrCode(40,m_GrCodeEnt,c_ValAsocEnt))]; 
iTex = iTex+1; 
end 
